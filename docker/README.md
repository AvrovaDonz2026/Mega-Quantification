...
| 机器 | 建议 |
|---|---|
| 单卡 5090 32 GB + 64 GB RAM | `MEGAQUANT_LOW_MEMORY=1` + `recipes/qwen3.8-27b-nvfp4-w4a8.5090.yaml` / `w4a4.5090.yaml` / `mixed.5090.yaml`。默认打满 VRAM−2GiB、内存 MemTotal−6GiB、全部 CPU 线程、`batch_size=4`。BF16 拷到本地盘，不要打 UPFS `/model`。 |
