...
如果你的 5090 已经在 Compshare / AutoDL 这类 **K8s GPU 容器**里（没有 Docker、系统盘只有几十 GB、权重在 `/model`），不要硬套 Compose，改用 `bash scripts/gpu-pod.sh plan` / `quantize`。
...
| 机器 | 建议 |
|---|---|
| 单卡 5090 32 GB + 64 GB RAM | `MEGAQUANT_LOW_MEMORY=1` + `recipes/qwen3.8-27b-nvfp4-w4a8.5090.yaml`。默认打满 VRAM−2GiB、内存 MemTotal−6GiB、全部 CPU 线程、`batch_size=4`。BF16 拷到本地盘，不要打 UPFS `/model`。 |
